...
TEXT = {
    "ru": {
        ...
        "age_range": "Возраст должен быть 18–99.",
        ...
        "match": "Матч.",
        "contact": "Контакт:",
        "no_username": "У пользователя нет @username — открой профиль в Telegram.",
    ...
    "en": {
        ...
        "match": "Match.",
        "contact": "Contact:",
        "no_username": "No username — open Telegram profile.",
...
def allowed_genders_by_looking(looking: str) -> tuple[str, ...]:
    # looking=men -> show m+n
    # looking=women -> show f+n
    # looking=all -> show m+f+n
    if looking == "m":
        return ("m", "n")
    if looking == "f":
        return ("f", "n")
...
async def admin_new_profile(uid: int):
    try:
        ...
        header = f"{TEXT['ru']['admin_new_profile']} • user_id: {uid}"
        if username:
            header += f" • @{username}"
...
async def menu_create(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("INSERT OR REPLACE INTO state(user_id, step) VALUES(?,?)", (call.from_user.id, "name"))
        await db.commit()
...
@dp.callback_query(F.data.in_({"l_m", "l_f", "l_all", "el_m", "el_f", "el_all"}))
async def cb_looking(call: CallbackQuery):
    lang = lang_of(getattr(call.from_user, "language_code", None))
    await set_user(call.from_user.id, call.from_user.username, lang)
    t = TEXT[lang]
    await call.answer()
    ...
    mapping = {
        "l_m": "m", "l_f": "f", "l_all": "all",
        "el_m": "m", "el_f": "f", "el_all": "all",
    }
    looking = mapping[call.data]

    async with aiosqlite.connect(DB_PATH) as db:
        if step == "looking":
            await db.execute("UPDATE draft SET looking=? WHERE user_id=?", (looking, call.from_user.id))
            await db.execute("UPDATE state SET step='photo' WHERE user_id=?", (call.from_user.id,))
            await db.commit()
            await call.message.answer(t["ask_photo"])
        else:
            await db.execute("UPDATE profiles SET looking=? WHERE user_id=?", (looking, call.from_user.id))
            await db.execute("DELETE FROM state WHERE user_id=?", (call.from_user.id,))
            await db.commit()
            await call.message.answer(t["updated"])
            await menu_send(call.message.chat.id, lang)
...
async def cb_like(call: CallbackQuery):
    ...
    await notify_like(target_id, call.from_user.id)
...
async def cb_report(call: CallbackQuery):
    ...
    await admin_report(call.from_user.id, target_id)
...
# =========================
# MESSAGE ROUTER (create/edit/comment)
# =========================
@dp.message()
async def router(message: Message):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if step == "age":
            ...
            age = int(s)
            if age < 18 or age > 99:
                await message.answer(t["age_range"])
                return
        ...
        if step == "bio":
            ...
            # Move draft -> profiles
            await db.execute("""
                INSERT OR REPLACE INTO profiles(user_id, name, age, city, gender, looking, photo_file_id, bio)
